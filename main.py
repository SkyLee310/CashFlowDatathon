...
def objective(trial):
    # Expanded parameter space to control complexity vs volatility
    param = {
        'n_estimators': 2000, # Set high, use early stopping to find the sweet spot
        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
        'max_depth': trial.suggest_int('max_depth', 3, 10),
        'min_child_weight': trial.suggest_int('min_child_weight', 1, 10), # Prevents overfitting to noise
        'subsample': trial.suggest_float('subsample', 0.5, 0.9),
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.5, 0.9),
        'gamma': trial.suggest_float('gamma', 1e-8, 1.0, log=True), # Minimum loss reduction to split
        'alpha': trial.suggest_float('alpha', 1e-8, 1.0, log=True), # L1 Reg
        'lambda': trial.suggest_float('lambda', 1e-8, 1.0, log=True), # L2 Reg
        'random_state': 42,
        'n_jobs': -1,
        'early_stopping_rounds': 50 # Stops training if error stops improving
    }
    ...
    for train_idx, val_idx in tscv.split(X_opt):
        ...
        # Evaluate on the REAL dollar scale (expm1), not just the log scale
        # This helps Optuna minimize actual financial error
